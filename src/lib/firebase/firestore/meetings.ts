...
// Firestore data converter for Meeting
...
export async function getMeetingsForUser(userId: string): Promise<Meeting[]> {
  ...
  // Order by meeting date, upcoming first
  const q = query(meetingsCol, orderBy("dateTime", "asc")); 
  const snapshot = await getDocs(q);
  return snapshot.docs.map(doc => doc.data());
}

export async function addMeetingForUser(userId: string, meetingData: Omit<Meeting, 'id' | 'createdAt' | 'updatedAt'>): Promise<Meeting> {
...
// Update and Delete functions can be added if needed for future functionality
export async function updateMeetingForUser(userId: string, meetingId: string, meetingData: Partial<Omit<Meeting, 'id' | 'createdAt' | 'updatedAt'>>): Promise<void> {
